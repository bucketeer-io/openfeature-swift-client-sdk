...
typealias OnInitializeCallback = ((Bucketeer.BKTConfig, Bucketeer.BKTUser, (((any BucketeerProtocol)?, Bucketeer.BKTError?) -> Void)) -> Void)
...
class MockBucketeerDI: BucketeerDI {
    ...
    func initialize(config: Bucketeer.BKTConfig, user: Bucketeer.BKTUser, completion: @escaping (((any BucketeerProtocol)?, Bucketeer.BKTError?) -> Void)) throws {
...
class MockBucketeerClient: BucketeerProtocol {
    ...
    func objectVariationDetails(featureId: String, defaultValue: Bucketeer.BKTValue) -> Bucketeer.BKTEvaluationDetails<Bucketeer.BKTValue> {
